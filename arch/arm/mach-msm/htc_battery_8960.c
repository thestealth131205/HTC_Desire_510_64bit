...
#define HTC_DEFAUL_CRITICAL_LOW_DROP	6
	...
	if (!prev_batt_info_rep->charging_enabled &&
		...
		if (is_voltage_critical_low(htc_batt_info.rep.batt_vol)) {
			...
			if (htc_batt_info.decreased_batt_level_check)
				...
				dec_level = htc_batt_info.critical_low_level_drop;
...
static int htc_battery_probe(struct platform_device *pdev)
{
	...
	htc_batt_info.critical_low_level_drop =
					pdata->critical_low_level_drop ? : HTC_DEFAUL_CRITICAL_LOW_DROP;
