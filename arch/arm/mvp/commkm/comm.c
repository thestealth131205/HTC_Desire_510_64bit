	      ...
	      void *arg2)
{
	...
	(void)arg1;
	(void)arg2;
	commos_debug("%s: running [%d] commChannelAllocated [%u] commChannelSize [%u].\n",
		      __func__, running,
...
Comm_Finish(unsigned long long *timeoutMillis)
{
	...
	if (rc == 1) {
		...
		commChannelSize = 0;
#if defined(COMM_INSTRUMENT)
		commos_info("%s: commMaxCoalesceSize = %lu.\n",
			    __func__,
			    CommOS_ReadAtomic(&commMaxCoalesceSize));
		commos_info("%s: commPacketsReceived = %lu.\n",
			    __func__,
			    CommOS_ReadAtomic(&commPacketsReceived));
		commos_info("%s: commCommittedPacketsReceived = %lu.\n",
			    __func__,
			    CommOS_ReadAtomic(&commCommittedPacketsReceived));
		commos_info("%s: commOpCalls = %lu.\n",
			    __func__,
	...
	if (CommIsActive(channel)) {
		...
		for (vecLen = 0; vecLen < VEC_SIZE; vecLen++) {
			...
			if (!vec[vecLen].iov_base) {
				commos_info("%s: BOOG -- COULD NOT DEQUEUE PAYLOAD! [%d != %u]",
					    __func__, rc, dataLen);
				rc = -1; 
				break;
			}
				...
				if ((rc == sizeof(packet)) &&
				   !CommTransp_EnqueueCommit(channel->transp)) {
					...
					if (CommIsOpened(channel)) {
						commos_debug("%s: Sent ack. Activating channel.\n",
							     __func__);
