...
DestroyTransp(CommTransp transp)
{
	...
	commos_info("%s: Detaching channel [%u:%u]\n",
		    __func__,
	...
	CommOS_Kfree(transp);
	if (rc != QP_SUCCESS)
		commos_info("%s: Failed to detach. rc: %d\n", __func__, rc);
	else
		commos_info("%s: Channel detached.\n", __func__);
}
		...
		CommTranspEvent *transpEvent)
{
	...
	commos_info("%s: Attaching to [%u:%u]. Capacity: %u\n",
	...
	if (TranspTableAdd(transpOut)) {
		commos_info("%s: Exceeded max limit of transport objects!\n",
			    __func__);
	...
	*transp = transpOut;
	...
	commos_info("%s: Channel attached.\n", __func__);

out:
	if (rc && transpOut) {
		commos_info("%s: Failed to attach: %d\n", __func__, rc);
...
CommTransp_DequeueCommit(CommTransp transp)
{
	...
	if (rc >= 0) {
		...
		if ((readable >= 0) &&
		    (readSize + (unsigned int)readable >= limit) &&
