...
PvtcpStateAlloc(CommChannel channel)
{
	...
	if (state) {
		...
		state->mask =
			((unsigned int)channel << 4) ^ (unsigned int)state;
...
PvtcpCheckArgs(CommTranspInitArgs *transpArgs)
{
	...
	while (versionIndex--) {
		if (transpArgs->type ==
		    CommTransp_GetType(pvtcpVersions[versionIndex])) {
	      ...
	      int inBH)
{
	...
	commos_info("%s: Channel was reset!\n", __func__);


	if (impl && !impl->owner && !inBH) {
		commos_info("%s: Attempting to re-initialize channel.\n",
			    __func__);
		...
		if (CommSvc_Alloc(transpArgs, impl, inBH, &pvtcpClientChannel))
			commos_info("%s: Failed to initialize channel!\n",
				    __func__);
	}
}
