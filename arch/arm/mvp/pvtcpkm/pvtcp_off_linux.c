...
PvsockNfHook(struct sk_buff *skb,
	     int inet6)
{
	kuid_t uid;
	unsigned int port;
	...
	uid = (sock->file ? sock->file->f_cred->uid : GLOBAL_ROOT_UID);
	if (uid_eq(uid, GLOBAL_ROOT_UID) || (sock->type != SOCK_STREAM && sock->type != SOCK_DGRAM))
		return NF_ACCEPT;

	if (likely(uid_eq(uid, Mvpkm_vmwareUid))) {
...
#else
Inet4NfHook(const struct nf_hook_ops *ops,
#endif
	    struct sk_buff *skb,
...
#else
Inet6NfHook(const struct nf_hook_ops *ops,
#endif
	    struct sk_buff *skb,
	    const struct net_device *in,
	    const struct net_device *out,
...
typedef struct  PvtcpStateKObjAttr {
	...
	ssize_t (*store)(PvtcpStateKObj *stateKObj, const char *buf,
			 size_t count);
} PvtcpStateKObjAttr;
...
StateKObjShow(struct kobject *kobj,
	      ...
	      char *buf)
{
	PvtcpStateKObjAttr *stateAttr =
		container_of(attr, PvtcpStateKObjAttr, attr);
		      ...
		      char *buf)
{
	...
	typeHash =
		CommTransp_GetType(pvtcpVersions[stateKObj->transpArgs.type]);
			...
			char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "%u.%u.%u.%u\n",
			...
			(unsigned int)addr.bytes[1],
			(unsigned int)addr.bytes[2],
		      ...
		      unsigned int portIdx)
{
	...
	rc = PvtcpTestLoopbackBit(loopbackAddrs[*((unsigned char *)&addr + 3)],
				  BIT(portIdx));
	return rc;
}
...
GetLoopbackAddr(void)
{
	...
	for (idx = 1; idx < PVTCP_OFF_MAX_LB_ADDRS; idx++) {
		if (!PvtcpTestLoopbackBit(loopbackAddrs[idx],
					  loopbackReserved)) {
			...
			if (!sock_create_kern(AF_INET, SOCK_DGRAM, 0, &sock)) {
				...
				snprintf(ifr.ifr_name, sizeof(ifr.ifr_name),
					 "lo:%u", idx);
				memcpy(&ifr.ifr_addr, &sin,
				       sizeof(ifr.ifr_addr));
				err = kernel_sock_ioctl(sock, SIOCSIFADDR,
							(unsigned long)&ifr);
				sock_release(sock);
				if (err) {
					commos_info("%s: Could not set loopbackaddress (ioctl)\n",
						    __func__);
					rc = -1U;
					continue; 
				} else {
					PvtcpSetLoopbackBit(loopbackAddrs[idx],
							    loopbackReserved);
				...
				commos_info("%s: Could not set loopback address (create)!\n",
					    __func__);
				rc = -1U;
				break;
			}
	...
	if (idx == PVTCP_OFF_MAX_LB_ADDRS)
		commos_info("%s: loopback address range exceeded!\n",
			    __func__);
...
PutLoopbackAddr(unsigned int uaddr)
{
	...
	if (!sock_create_kern(AF_INET, SOCK_DGRAM, 0, &sock)) {
		...
		commos_info("%s: Could not delete loopback address!\n",
			    __func__);
	}
...
GetNetNamespace(PvtcpState *state)
{
	...
	struct msghdr msg = {
		...
		.msg_namelen = 1 + offsetof(struct sockaddr_un, sun_path) +
			       socknamelen
	};
...
StateAlloc(CommChannel channel)
{
	...
	if (loopbackNetif->conf.addr.in.s_addr == -1U) {
		commos_info("%s: Could not allocate loopback address!\n",
			    __func__);
		goto error;
	}
			      ...
			      unsigned short port)
{
	...
	if (rc) {
		...
		if ((SkFromPvsk(pvsk)->sk_socket->type == SOCK_DGRAM) &&
		    (port < portRangeSize)) {
			CommOS_MutexLock(&globalLock);
			PvtcpSetPortIndexBit(pvsk->netif->conf.addr.in.s_addr,
					     port);
			CommOS_MutexUnlock(&globalLock);
		}
			      ...
			      unsigned short port)
{
	...
	if (pvsk->netif->conf.family == PVTCP_PF_LOOPBACK_INET4) {
		...
		ipv6_addr_set_v4mapped(pvsk->netif->conf.addr.in.s_addr,
				       &in6Addr.in6);
		rc = 0;
		goto out;
	}
			...
			struct in6_addr *in6)
{
	if (!PvtcpHasSockNamespace(pvsk) && ipv6_addr_v4mapped(in6)) {
		if (PvskTestFlag(pvsk, PVTCP_OFF_PVSKF_IPV6_LOOP)) {
			...
			static const struct in6_addr in6Loopback =
...
Init(void *args)
{
	...
	if (rc) {
		commos_info("%s: Could not register netfilter hooks!\n",
			    __func__);
		goto out;
	} else {
	...
	hooksRegistered = 1;
#else
	commos_info("%s: Netfilter hooks disabled.\n", __func__);
#endif
	...
	if (CommSvc_RegisterImpl(&pvtcpImpl) == 0) {
		...
		if (pvtcpLoopbackOffAddr == -1U) {
			commos_info("%s: Could not allocate offload loopback address!\n",
				    __func__);
			rc = -1;
...
out:
	if (rc) {
		if (hooksRegistered)
			nf_unregister_hooks(netfilterHooks,
					    ARRAY_SIZE(netfilterHooks));
	}
	return rc;
}
...
#endif
	commos_info("%s: Allocations of large datagrams: %llu.\n",
	      ...
	      PvtcpSock *parentPvsk)
{
	...
	if (!parentPvsk) {
		...
		if (parentPvsk->netif->conf.family == PVTCP_PF_LOOPBACK_INET4) {
			...
			PvtcpSwitchSock(pvsk, PVTCP_SOCK_NAMESPACE_INITIAL);
			PvtcpStateAddSocket(pvsk->channel,
					    pvtcpIfLoopbackInet4, pvsk);
		}
	}
	      ...
	      unsigned int vecLen)
{
	...
	if (!rc) {
		...
		if (state->extra && ((PvtcpStateKObj *)(state->extra))->useNS)
			PvtcpSwitchSock(pvsk, PVTCP_SOCK_NAMESPACE_CHANNEL);
		else
			PvtcpSwitchSock(pvsk, PVTCP_SOCK_NAMESPACE_INITIAL);
...
fail:
	commos_info("%s: BOOG ** FAILED TO CREATE OFFLOAD SOCKET [%d] _AND_ ERROR REPORTING SOCKET!\n"
	       ...
	       unsigned int vecLen)
{
	...
	if ((sk->sk_socket->type == SOCK_DGRAM) &&
		...
		if (port < portRangeSize) {
			CommOS_MutexLock(&globalLock);
			PvtcpResetPortIndexBit(pvsk->netif->conf.addr.in.s_addr,
					       port);
			CommOS_MutexUnlock(&globalLock);
		}
	}
		  ...
		  unsigned int vecLen)
{
	...
	if ((vecLen != 1) || (vec[0].iov_len != optlen) ||
	    (optlen < sizeof(int))) {
...
CreateAIO(PvtcpSock *pvsk)
{
	...
	if (rc != packet.len) {
		...
		PvtcpStateRemoveSocket(pvsk->channel, pvsk);
		SockReleaseWrapper(sock);
		commos_info("%s: BOOG -- Couldn't send 'Create' reply [0x%p]\n",
			    __func__, sk);
	} else {
#if defined(PVTCP_FULL_DEBUG)
	...
	if (pvsk->peerSockSet) {
		if (sk->sk_family == AF_INET) {
			...
			if (rc == 0) {
				...
				PvtcpResetLoopbackInet4(pvsk,
							&sin.sin_addr.s_addr);
				packet.data64ex =
					(unsigned long long)sin.sin_addr.s_addr;
			}
		} else { 
			struct sockaddr_in6 sin = { .sin6_family = AF_INET6 };
			int addrLen = sizeof(sin);
...
AcceptAIO(PvtcpSock *pvsk)
{
	...
	if (pvsk->peerSockSet) {
		...
		packet.data64 = pvsk->parent->peerSock;
...
PvtcpProcessAIO(CommOSWork *arg)
{
	...
	if (!SOCK_IN_TRYLOCK(pvsk)) {
		...
		if (sk && sk->sk_socket) {
			...
			err = PvtcpInputAIO(pvsk,
					    perCpuBuf[smp_processor_id()]);
...
doneInUnlock:
		...
		if ((pvsk->peerSockSet ||
		     PvskTestOpFlag(pvsk, PVTCP_OP_RELEASE)) &&
		    sk && sk->sk_socket)
			PvtcpSchedSock(pvsk);
	}
