	...
	if (sk->sk_type == SOCK_STREAM) {
		...
		if (!SOCK_OUT_TRYLOCK(pvsk)) {
			if (pvsk->peerSockSet &&
				...
				struct kvec *vecTmp =
					kmemdup(vec, vecLen * sizeof(*vec),
						GFP_ATOMIC);
				if (vecTmp) {
					msg.msg_flags =
						MSG_DONTWAIT | MSG_NOSIGNAL;
					rc = kernel_sendmsg(sock, &msg, vecTmp,
							    vecLen, dataLen);
					kfree(vecTmp);
...
enqueueBytes:
			...
			if (pvsk->peerSockSet &&
				...
				if (queueSize > 0) {
					...
					CommOS_ListSpliceTail(&pvsk->queue,
							      &bufList);
					SOCK_STATE_UNLOCK(pvsk);
					CommOS_AddReturnAtomic(&pvsk->queueSize,
							       queueSize);
					needSched = 1;
				}
			} else {
		...
		if (sk->sk_family == AF_INET) {
			...
			PvtcpTestAndBindLoopbackInet4(pvsk,
	...
	if (pvsk->err || err) {
		...
		if (CommOS_ReadAtomic(&pvsk->sentSize) >= tmpSize) {
			if ((SkFromPvsk(pvsk)->sk_type != SOCK_STREAM) &&
				...
				packet.data32 =
					CommOS_ReadAtomic(&pvsk->sentSize);
				CommOS_WriteAtomic(&pvsk->sentSize, 0);
				if (tmpSize >
				    (1 << (PVTCP_SOCK_SMALL_ACK_ORDER + 1))) {
					tmpSize >>= 1;
	...
	while (!done && CommOS_ReadAtomic(&pvsk->queueSize) > 0) {
		...
		if (sk->sk_state == TCP_ESTABLISHED) {
			CommOS_ListForEach(&queue, internalBuf, link) {
				...
				vec[vecLen].iov_base =
					PvtcpOffBufFromInternalOff(internalBuf);
			...
			if (rc >= 0) {
				...
				if (queueDelta == 0) {
					/*
					 * Bail out if no bytes written,
					 * WriteSpaceCB() will resched.
					 */

					done = 1;
					break;
				}
				CommOS_AddReturnAtomic(&pvsk->sentSize,
						       queueDelta);
				CommOS_SubReturnAtomic(&pvsk->queueSize,
						       queueDelta);
			} else {
...
discardOutput:
			if (!CommOS_ListEmpty(&queue)) {
				CommOS_ListForEachSafe(&queue, internalBuf,
						       tmp, link) {
					CommOS_ListDel(&internalBuf->link);
					PvtcpBufFree(PvtcpOffBufFromInternal(internalBuf));
				}
	      ...
	      void *perCpuBuf)
{
	...
	if (sk->sk_state == TCP_LISTEN) {
		...
		while ((tmpSize < PVTCP_SOCK_SAFE_RCVSIZE) &&
		       pvsk->peerSockSet) {
			if (ioBuf != perCpuBuf) {
			...
			if (sk->sk_type == SOCK_STREAM) {
				...
				rc = kernel_recvmsg(sock, &msg, vec,
						    1, 1, msg.msg_flags);
				if (rc < 0)
					break;
			...
			rc = kernel_recvmsg(sock, &msg, vec, 1,
					    vec[0].iov_len, msg.msg_flags);
			if (rc < 0)
				break;
			...
			if (tmpSize >= PVTCP_SOCK_LARGE_ACK_WM)
				COMM_OPF_SET_VAL(packet.flags,
						 PVTCP_SOCK_LARGE_ACK_ORDER);
			else
			...
			if (sk->sk_type == SOCK_STREAM) {
				...
				if (sk->sk_family == AF_INET) {
					dgramHeader.d0 =
					    (unsigned long long)sin.sin_port;
					...
					dgramHeader.d0 =
					    (unsigned long long)sin6.sin6_port;
					PvtcpResetLoopbackInet6(pvsk,
								&sin6.sin6_addr);
				...
				inVecLen = 2;
				packet.len =
				    sizeof(packet) + sizeof(dgramHeader) + rc;
			}
			...
			rc = CommSvc_WriteVec(pvsk->channel, &packet,
					      ...
					      &timeout, &iovOffset, 1);
			if (rc != packet.len) {
				commos_info("%s: BOOG -- WROTE INCOMPLETE PACKET [%u->%d]!\n",
