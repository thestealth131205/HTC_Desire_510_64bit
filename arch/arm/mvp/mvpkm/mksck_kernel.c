...
Mksck_Init(void)
{
	...
	if (err != 0) {
		pr_err("Mksck_Init: Cannot register AF_MKSCK protocol, errno = %d.\n",
		       err);
		return err;
	}
	...
	if (err < 0) {
		pr_err("Mksck_Init: Could not register address family AF_MKSCK (errno = %d).\n",
		       err);
		return err;
	}
...
{
	struct sock *sk;
	kuid_t currentUid = current_euid();

	if (!(uid_eq(currentUid, GLOBAL_ROOT_UID) ||
	    uid_eq(currentUid, Mvpkm_vmwareUid))) {
		pr_warn("MksckCreate: rejected from process %s tgid=%d, pid=%d euid:%d.\n",
			current->comm,
			task_tgid_vnr(current),
			task_pid_vnr(current),
			from_kuid(current_user_ns(), currentUid));
		return -EPERM;
	}
		 ...
		 Mksck_Address addr)
{
	...
	pr_debug("MksckBind: socket bound to %08X\n",
		 mksck->addr.addr);
		  ...
		  int flags)
{
	...
	if (peerAddrMk->mk_addr.addr != MKSCK_ADDR_UNDEF) {
		...
		pr_debug("MksckConnect: socket %x is connected to %x!\n",
			      ...
			      poll_table *wait)
{
	...
	if (err < 0) {
		pr_debug("MksckPoll: try to abort\n");
		return mask;
	}
	...
	for (i = 0; i < pages && pd[i].mpn != INVALID_MPN; ++i) {
		...
		for (j = 0; j < 1 << pd[i].order; ++j) {
			...
			if (!pfn_valid(currMPN)) {
				pr_warn("MksckPageDescManage: Invalid MPN %"LONG_FORMAT"\n",
					currMPN);
			} else {
		 ...
		 struct vm_area_struct *vma)
{
	...
	for (i = 0; i < pages && pd[i].mpn != INVALID_MPN; ++i) {
		...
		for (j = 0; j < 1 << pd[i].order; ++j) {
			...
			if (iovCount == 0) {
				pr_warn("MksckPageDescMap: Invalid iov length\n");
				goto map_done;
			}
			...
			if (huva & (PAGE_SIZE - 1) ||
				...
				pr_warn("MksckPageDescMap: Invalid huva %"LONG_FORMAT" or iov_len %"SIZE_T_FORMAT"\n",
					huva, iov->iov_len);
				goto map_done;
			}
			...
			if (vma == NULL || huva < vma->vm_start ||
				...
				if (vma == NULL ||
					...
					pr_warn("MksckPageDescMap: Invalid vma\n");
					goto map_done;
				}
			}

			if (!pfn_valid(currMPN)) {
				pr_warn("MksckPageDescMap: Invalid MPN %"LONG_FORMAT"\n",
					currMPN);
			} else {
				int rc;
				...
				if (rc) {
					pr_warn("MksckPageDescMap: Failed to insert %"LONG_FORMAT" at %"LONG_FORMAT", error %d\n",
						currMPN, huva, rc);
					goto map_done;
				}
		  ...
		  int flags)
{
	...
	while (1) {
		...
		err = Mutex_UnlSleep(&mksck->mutex, MutexModeEX,
				     MKSCK_CVAR_FILL);
		if (err < 0) {
			pr_debug("MksckDgramRecvMsg: aborted\n");
			goto decRefc;
		}
	}
	...
	if (untypedLen <= len) {
		...
		if (err < 0) {
			pr_warn("MksckDgramRecvMsg: Failed to memcpy_to_iovec untyped message component (buf len %"SIZE_T_FORMAT" datagram len %d (untyped %"SIZE_T_FORMAT"))\n",
		  ...
		  size_t len)
{
	...
	while (1) {
		...
		err = Mutex_UnlSleep(&peerMksck->mutex, MutexModeEX,
				     MKSCK_CVAR_ROOM);
		if (err < 0) {
			pr_debug("MksckDgramSendMsg: aborted\n");
			goto decRefc;
		}
	}
...
void
Mksck_WspRelease(WorldSwitchPageType *wsp)
{
	int ii;
	int err;
	...
	do {
		while ((oldRefc = ATOMIC_GETO(mksckPage->refCount)) == 1) {
			...
			if (ATOMIC_SETIF(mksckPage->refCount, 0, 1)) {
				...
				mksckPages[ii] = NULL;
				spin_unlock(&mksckPageListLock);
				pr_warn("%s mksck page is released: idx %x, vmId %x, tgid %d\n",
