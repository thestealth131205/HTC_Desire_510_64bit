...
AllocZeroedFreePages(struct MvpkmVM *vm,
		     ...
		     PhysMem_RegionType forRegion,
		     HKVA *hkvaRet);
static HKVA  MapWSPHKVA(struct MvpkmVM *vm,
		uint32 nrPages,
		HkvaMapInfo *mapInfo);
...
kuid_t Mvpkm_vmwareUid = INVALID_UID;
EXPORT_SYMBOL(Mvpkm_vmwareUid);
kgid_t Mvpkm_vmwareGid = INVALID_GID;
...
{
#ifdef CONFIG_ARM64
	const char aarchVersion[] = MVP_AARCH64_VERSION_STR;
#else
	const char aarchVersion[] = MVP_AARCH32_VERSION_STR;
#endif
#ifdef CONFIG_ARM_LPAE
	const char lpaeSuffix[] = MVP_LPAE_VERSION_STR;
#else
	const char lpaeSuffix[] = "";
#endif
	return snprintf(buf, PAGE_SIZE,
			MVP_VERSION_FORMATSTR "%s%s\n",
			MVP_VERSION_FORMATARGS, aarchVersion, lpaeSuffix);
}
...
#define LOWMEMKILLER_SHRINK_MD5 0
	...
	if (global_page_state(NR_SHMEM) + total_swapcache_pages <
		...
		total_swapcache_pages < lobal_page_state(NR_FILE_PAGES))
                other_file = global_page_state(NR_FILE_PAGES) -
                             global_page_state(NR_SHMEM) -
...
#ifdef CONFIG_COMPAT 
	.compat_ioctl     = MvpkmUnlockedIoctl,
#endif
	.open             = MvpkmOpen,
	  ...
	  struct file *filp)
{
	...
	if (uid_eq(Mvpkm_vmwareUid, INVALID_UID))
	   ...
	   struct vm_fault *vmf)
{
	...
	if (!pfn_valid(mpn)) {
		pr_err("MvpkmMMap: Failed to insert %"LONG_FORMAT" @ %lx, mpn invalid\n",
		       mpn, address);
	} else if (LockedListLookup(vm, mpn)) {
		if (vm_insert_page(vma, address, pfn_to_page(mpn)) == 0)
			return VM_FAULT_NOPAGE;

		pr_err("MvpkmMMap: Failed to insert %"LONG_FORMAT" @ %lx\n",
		       mpn, address);
	...
	if (vm->stubPageMPN) {
		if (vm_insert_page(vma, address,
			...
			pr_info("MvpkmMMap: mapped the stub page at %"LONG_FORMAT" @ %lx\n",
				mpn, address);
			return VM_FAULT_NOPAGE;
		}

		pr_err("MvpkmMMap: Could not insert stub page %"LONG_FORMAT" @ %lx\n",
		       mpn, address);
	}
...
#else
static void
DetermineMemAttr64(ARM_ARM64_L3D *attribL3D, ARM_MemAttrNormal *attribMAN)
{
	HKVA hkva = __get_free_pages(GFP_KERNEL, 0);

	ARM_ARM64_L3D *pt = (ARM_ARM64_L3D *)hkva;
	ARM_ARM64_L3D *kernL3D = &pt[0], *userL3D = &pt[1];
	uint64 attr, mair;

	set_pte((pte_t *)kernL3D, pfn_pte(0, PAGE_KERNEL));
	set_pte((pte_t *)userL3D, pfn_pte(0, PAGE_NONE));

	pr_info("DetermineMemAttr: Kernel L3D AttrIndx=%x SH=%x\n",
		kernL3D->block.attrIndx, kernL3D->block.sh);

	pr_info("DetermineMemAttr: User   L3D AttrIndx=%x SH=%x\n",
		userL3D->block.attrIndx, userL3D->block.sh);

	ASSERT(kernL3D->block.attrIndx == userL3D->block.attrIndx);
	ASSERT(kernL3D->block.sh == userL3D->block.sh);

	*attribL3D = *kernL3D;

	switch (kernL3D->block.sh) {
	case 0:
		attribMAN->share = ARM_SHARE_ATTR_NONE;
		break;
	case 2:
	...
	attr = MVP_EXTRACT_FIELD64(mair,
				 8 * kernL3D->block.attrIndx,
				 8);
...
#define MAIR_ATTR_2_CACHE_ATTR(x, y) \
	...
	MAIR_ATTR_2_CACHE_ATTR(MVP_EXTRACT_FIELD64(attr, 2, 2),
			       attribMAN->innerCache);
	MAIR_ATTR_2_CACHE_ATTR(MVP_EXTRACT_FIELD64(attr, 6, 2),
			       attribMAN->outerCache);
		   ...
		   unsigned long arg)
{
	...
	case MVPKM_MAP_WSPHKVA: {
		...
		mapInfo = kmalloc(mvpkmMapInfo.nrPages * sizeof(*mapInfo),
				GFP_TEMPORARY);
		if (!mapInfo)
			return -ENOMEM;
		if (copy_from_user(mapInfo, (void *)mvpkmMapInfo.mapInfo,
				   mvpkmMapInfo.nrPages * sizeof(*mapInfo))) {
			kfree(mapInfo);
			return -EFAULT;
		}

		mvpkmMapInfo.hkva = MapWSPHKVA(vm, mvpkmMapInfo.nrPages, mapInfo);
		kfree(mapInfo);
		BUG_ON(mvpkmMapInfo.hkva == 0);

		if (mvpkmMapInfo.forRegion == MEMREGION_WSP)
#ifdef CONFIG_ARM64
			vm->wsp = (WorldSwitchPageType *) mvpkmMapInfo.hkva;
#else
			vm->wsp = (WorldSwitchPageType *) ((uint32)mvpkmMapInfo.hkva);
#endif
	...
	case MVPKM_ABORT_MONITOR:
		...
		break;
#ifdef CONFIG_ARM64
	case MVPKM_CPU_INFO64: {
		struct MvpkmCpuInfo64 buf;
#ifndef CONFIG_ARM64
		pr_err("MvpkmIoctl: MVPKM_CPU_INFO64 not supported on Aarch32\n");
		FATAL();
#else
		uint64 mpidr;
		uint64 mTcrEl1;
		...
		if ((mTcrEl1 & TCR_IPS_MASK) != TCR_IPS_40BIT) {
			pr_err("VMM Only support 40 bits physical address. TCR_EL1 = %llx\n", mTcrEl1);
			FATAL();
		}

		DetermineMemAttr64(&buf.attribL3D, &buf.attribMAN);

		asm volatile ("mrs %0, MPIDR_EL1 \n\t" : "=r" (mpidr) :);
		buf.mpExt = mpidr & (1<<30);
#endif
		if (copy_to_user((int *)arg, &buf, sizeof(buf)))
			retval = -EFAULT;
		break; }
#endif
	case MVPKM_CPU_INFO: {
...
#else
		pr_err("MvpkmIoctl: MVPKM_CPU_INFO not supported on Aarch64\n");
		FATAL();
#endif
	...
	pr_debug("Returning from IOCTL(%d) retval = %d %s\n",
		 cmd, retval,
		 signal_pending(current) ? "(pending signal)" : "");
		     ...
		     HKVA *hkvaRet)
{
	...
	if (order > PAGE_ALLOC_COSTLY_ORDER)
		pr_warn("Order %d allocation for region %d exceeds the safe maximum order %d\n",
			order,
			forRegion,
	   ...
	   HkvaMapInfo *mapInfo)
{
	...
	for (i = pageIndex; i < pageIndex + nrPages; i++)
...
{
	int retval;
	WorldSwitchPageType *wsp = vm->wsp;
...
#define USE_ARCH_EXTENSION_SEC 0
	...
	if (!wsp || wsp->wspHKVA != (typeof(wsp->wspHKVA))wsp)
		return -EINVAL;
...
static void
SetSwitchTimes(WorldSwitchPageType *wsp)
{
	...
	wsp->switchedAt64 = timespec_to_ns(&now);
}
...
RunMonitor(struct MvpkmVM *vm)
{
	...
	while (wsp->critSecCount > 0 ||
		...
		switch (wsp->monType) {
#ifdef CONFIG_ARM64
		case MONITOR_TYPE_LPV64: {
			MA hostVBAR;
			typedef void (*SwitchToMonitorCast)(RegisterSaveType *regSave);

			hostVBAR = ARM_ReadSystemRegister(ARM_MVP_VBAR);
			((SwitchToMonitorCast)(wsp->switchToMonitor.ptr64))(&wsp->regSave);
			ARM_WriteSystemRegister(ARM_MVP_VBAR, hostVBAR);
...
monitorExit:
	...
	if (ATOMIC_GETO(wsp->hostActions) & ACTION_ABORT) {
		pr_debug("Monitor has ABORT flag set.\n");
		...
		_Bool suspend)
{
	...
	if (suspend) {
		...
		if (ret == 0)
			pr_warn("MvpkmWaitForInt: guest stuck for 10s in WFI! (hostActions %08x)\n",
