...
#ifndef _ARM32_INLINE_H_
#define _ARM32_INLINE_H_

#include "arm32_gcc_inline.h"
...
ARM_ReadSystemRegister(uint8 reg)
{
	...
	case ARM_MVP_PERF_PMNC:
		ARM_MRC_CP15(PERF_MON_CONTROL_REGISTER, value);
		break;
	case ARM_MVP_PERF_PMCNT:
		ARM_MRC_CP15(PERF_MON_COUNT_SET, value);
		break;
	case ARM_MVP_VBAR:
		ARM_MRC_CP15(VECTOR_BASE, value);
		break;
	default:
		NOT_IMPLEMENTED();
		break;
			...
			uint32 value)
{
	switch (reg) {
	case ARM_MVP_PERF_PMNC:
		ARM_MCR_CP15(PERF_MON_CONTROL_REGISTER, value);
		break;
	case ARM_MVP_PERF_PMCNT:
		ARM_MCR_CP15(PERF_MON_COUNT_SET, value);
		break;
	case ARM_MVP_VBAR:
		ARM_MCR_CP15(VECTOR_BASE, value);
		break;
	default:
		NOT_IMPLEMENTED();
		break;
