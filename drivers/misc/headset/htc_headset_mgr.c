...
{
	int OEz = 0;
	if(!gpio_is_valid(htc_headset_mgr_data.oe_gpio))
		return;
...
static int headset_mgr_parser_dt(struct htc_hs_mgr_data *mgr)
{
	...
	const char *paser_alt   = {"mgr,wire_alt"},
			*parser_tx = {"mgr,tx_gpio"},
			*parser_rx = {"mgr,rx_gpio"},
			*parser_oe = {"mgr,oe_gpio"},
			*parser_bias = {"mgr,bias_gpio"},
	...
	htc_headset_mgr_data.tx_gpio = of_get_named_gpio(dt, parser_tx, 0);
		if (!gpio_is_valid(htc_headset_mgr_data.tx_gpio)) {
			HS_LOG("DT_mgr %s parser fail, ret = %d",parser_tx, htc_headset_mgr_data.tx_gpio);
			HS_LOG("NOT Support 1- wire\r\n");
		}

	htc_headset_mgr_data.rx_gpio = of_get_named_gpio(dt, parser_rx, 0);
		if (!gpio_is_valid(htc_headset_mgr_data.rx_gpio)) {
			HS_LOG("DT_mgr %s parser fail, ret = %d",parser_rx, htc_headset_mgr_data.rx_gpio);
			goto parser_fail;
		}

	htc_headset_mgr_data.oe_gpio = of_get_named_gpio(dt, parser_oe, 0);
		if (!gpio_is_valid(htc_headset_mgr_data.oe_gpio)) {
			HS_LOG("DT_mgr %s parser fail, ret = %d",parser_oe, htc_headset_mgr_data.oe_gpio);
			HS_LOG("NOT Support 1- wire\r\n");
		}
		else {
			gpio_request(htc_headset_mgr_data.oe_gpio, "headset_oe");
			if (ret < 0)
				printk("[HS] gpio_request headset_oe fail!\n");
			else
				printk("[HS] gpio_request headset_oe OK!\n");
		}

	htc_headset_mgr_data.bias_gpio = of_get_named_gpio(dt, parser_bias, 0);
		if (!gpio_is_valid(htc_headset_mgr_data.bias_gpio)) {
			HS_LOG("DT_mgr %s parser fail, ret = %d",parser_bias, htc_headset_mgr_data.bias_gpio);
			goto parser_fail;
		}

	gpio_request(htc_headset_mgr_data.bias_gpio, "headset_bias");
	if (ret < 0)
		printk("[HS] gpio_request headset_bias fail!\n");
	else
		printk("[HS] gpio_request headset_bias OK!\n");
	...
	if(!gpio_is_valid(htc_headset_mgr_data.oe_gpio))
			HS_LOG("DT:rx[%d],bias[%d]", htc_headset_mgr_data.rx_gpio,htc_headset_mgr_data.bias_gpio);
	else
			HS_LOG("DT:tx[%d],rx[%d],alt[%d],bias[%d],oe[%d]",htc_headset_mgr_data.tx_gpio, htc_headset_mgr_data.rx_gpio,
		htc_headset_mgr_data.wire_alt, htc_headset_mgr_data.bias_gpio, htc_headset_mgr_data.oe_gpio);
