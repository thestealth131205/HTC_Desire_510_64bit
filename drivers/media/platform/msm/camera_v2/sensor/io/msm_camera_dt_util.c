...
static int msm_camera_pinctrl_init(struct msm_camera_power_ctrl_t *ctrl)
{
	...
	sensor_pctrl->gpio_state_release
		= pinctrl_lookup_state(sensor_pctrl->pinctrl,
				CAM_SENSOR_PINCTRL_STATE_RELEASE);
    #if 0
	if (IS_ERR_OR_NULL(sensor_pctrl->gpio_state_release)) {
		pr_err("%s:%d Failed to get the release state pinctrl handle\n",
				__func__, __LINE__);
		return -EINVAL;
	}
    #endif
	...
	for (index = 0; index < ctrl->power_setting_size; index++) {
		...
		case SENSOR_VREG:
			...
			if (cam_vreg->type == REG_GPIO) {
				...
				pr_err("%s:%d SENSOR_VREG gpio set val %d\n", __func__, __LINE__,
					ctrl->gpio_conf->gpio_num_info->gpio_num
					[power_setting->seq_val]);
			}else{
			pr_err("msm_camera_config_single_vreg \n");
...
power_up_failed:
	...
	if (ctrl->cam_pinctrl_status) {
        if (IS_ERR_OR_NULL(ctrl->pinctrl_info.gpio_state_release)){
            ...
            ret = pinctrl_select_state(ctrl->pinctrl_info.pinctrl,
                    ctrl->pinctrl_info.gpio_state_release);
            if (ret)
                pr_err("%s:%d cannot set pin to release state\n",
                    __func__, __LINE__);
        }
	}
#endif
