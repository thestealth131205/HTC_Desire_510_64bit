	...
	struct msm_sensor_power_setting_array *power_setting_array)
{
	...
	if (rc < 0) {
		pr_err("%s failed: Parse and fill vreg params for powerup settings rc %d", __func__, rc);
		goto FREE_UP;
	}
	...
	if (rc < 0) {
		pr_err("%s failed: Parse and fill vreg params for powerdown settings rc %d", __func__, rc);
		goto FREE_UP;
	}
