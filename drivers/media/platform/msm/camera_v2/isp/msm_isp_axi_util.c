...
void msm_isp_sof_notify(struct vfe_device *vfe_dev,
	...
	if ((vfe_dev->axi_data.current_frame_src_mask[session_id] ==
		...
		if (frame_src == VFE_PIX_0) {
			if((vfe_dev->axi_data.frame_id[session_id] < 3) ||
			   (vfe_dev->axi_data.frame_id[session_id] == 10) ||
			   ((vfe_dev->axi_data.frame_id[session_id]%50) == 0)) {
				pr_info("[CAM]%s:(%d)PIX0 frame id: %d\n", __func__, session_id,
				vfe_dev->axi_data.frame_id[session_id]);
			}
		}
