...
static void configure_bark_dump(struct msm_watchdog_data *wdog_dd)
{
	...
	if (MSM_DUMP_MAJOR(msm_dump_table_version()) == 1) {
...
#if defined(CONFIG_HTC_DEBUG_WATCHDOG)
			cpu_dump_entry.start_addr = wdog_dd->scm_regsave_phys;
			cpu_dump_entry.end_addr = cpu_dump_entry.start_addr + wdog_dd->scm_regsave_size;
#else
		...
		return;
#endif
		if (wdog_dd->scm_regsave_phys) {
			cpu_data = ioremap(wdog_dd->scm_regsave_phys, sizeof(struct msm_dump_data) * num_present_cpus());
			if(!cpu_data) {
				pr_err("cpu dump data structure ioremap failed\n");
				goto out0;
			}
			memset(cpu_data, 0, sizeof(struct msm_dump_data) * num_present_cpus());

			cpu_buf_phys = wdog_dd->scm_regsave_phys + sizeof(struct msm_dump_data) * num_present_cpus();
			cpu_buf = ioremap(cpu_buf_phys, MAX_CPU_CTX_SIZE * num_present_cpus());
			if(!cpu_buf) {
				pr_err("cpu reg context space ioremap failed\n");
				goto out1;
			}
			memset(cpu_buf, 0, MAX_CPU_CTX_SIZE * num_present_cpus());
		}
#else
...
#if defined(CONFIG_HTC_DEBUG_WATCHDOG)
			cpu_data[cpu].addr = cpu_buf_phys + cpu * MAX_CPU_CTX_SIZE;
			cpu_data[cpu].len = MAX_CPU_CTX_SIZE;
			dump_entry.id = MSM_DUMP_DATA_CPU_CTX + cpu;
			dump_entry.addr = wdog_dd->scm_regsave_phys + cpu * sizeof(struct msm_dump_data);
#else
