...
struct wake_lock clear_kpdpwr_s2_wakelock;
#define KPDPWR_CLR_RESET_TIMER (150 * NSEC_PER_MSEC) 
#define KPDPWR_REBOOT_DELAY (6*HZ)
#endif
...
static struct gpio_event_input_info *gis_S2;
int set_restart_to_ramdump(const char *msg);

static void power_key_restart_work_func_S2(struct work_struct *dummy)
{
	uint8_t gpio_val = 0, i, idx_pwr = 9, idx_vup = 9;
	struct gpio_event_input_info *local = gis_S2;
	for (i = 0; i < local->keymap_size; i++) {
		if (local->keymap[i].code == KEY_POWER) {
			idx_pwr = i;
			continue;
...
static DECLARE_DELAYED_WORK(power_key_restart_work_S2, power_key_restart_work_func_S2);

static void handle_pmic_kpdpwr_s2_reset(unsigned int code, int value)
{
	if (code == KEY_POWER) {
		wake_lock_timeout(&clear_kpdpwr_s2_wakelock, msecs_to_jiffies(1000));
		if (value) {
			clear_kpdpwr_s2_rst_flag = 1;
                        KEY_LOGI("%s: Enable kpdpwr s2 reset clear up [%d]\n",
					__func__, clear_kpdpwr_s2_rst_flag);
			sw_mistouch_ctrl(1);
		} else {
			clear_kpdpwr_s2_rst_flag = 0;
                        KEY_LOGI("%s: Disable kpdpwr s2 reset clear up [%d]\n",
                                                __func__, clear_kpdpwr_s2_rst_flag);
			sw_mistouch_ctrl(0);
		}
	}
	...
	if (code == KEY_POWER || code == KEY_VOLUMEUP) {
		if (value) {
			schedule_delayed_work(&power_key_restart_work_S2, KPDPWR_REBOOT_DELAY);
		} else {
			cancel_delayed_work(&power_key_restart_work_S2);
		}
	}		
}
#endif
...
int sw_mistouch_ctrl(int en)
{
	...
	if (en) {
		ret = hrtimer_start(&clr_kpd_reset_timer,
			ktime_set(0, KPDPWR_CLR_RESET_TIMER), HRTIMER_MODE_REL);
	} else {
		if (hrtimer_is_queued(&clr_kpd_reset_timer))
			ret = hrtimer_cancel(&clr_kpd_reset_timer);
		if (hrtimer_is_queued(&enable_kpd_s2_timer))
			ret = hrtimer_cancel(&enable_kpd_s2_timer);
	}
...
static enum hrtimer_restart enable_kpd_s2_timer_func(struct hrtimer *timer)
{
	...
	if (clear_kpdpwr_s2_rst_flag) {
		...
		if (hrtimer_is_queued(&clr_kpd_reset_timer))
                ...
                if (hrtimer_is_queued(&enable_kpd_s2_timer))
                        hrtimer_cancel(&enable_kpd_s2_timer);
		wake_unlock(&clear_kpdpwr_s2_wakelock);
        }
...
#endif
#ifdef CONFIG_CLR_KPDPWR_RESET_BATT_RMV
		handle_pmic_kpdpwr_s2_reset(key_entry->code, pressed);
#endif
			...
			struct gpio_event_info *info, void **data, int func)
{
    ...
    defined(CONFIG_POWER_KEY_CLR_RESET)
		...
		wake_lock_init(&clear_kpdpwr_s2_wakelock, WAKE_LOCK_SUSPEND, "pwr_clear_reset");
#endif
		spin_unlock_irqrestore(&ds->irq_lock, irqflags);
		return 0;
	}
...
#if defined(CONFIG_CLR_KPDPWR_RESET_BATT_RMV) || \
    defined(CONFIG_POWER_KEY_CLR_RESET)
	wake_lock_destroy(&clear_kpdpwr_s2_wakelock);
#endif
	kfree(ds);
err_ds_alloc_failed:
	return ret;
