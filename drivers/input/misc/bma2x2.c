...
#define D(x...) printk(KERN_DEBUG "[GSNR][BMA2X2] " x)
#define I(x...) printk(KERN_INFO "[GSNR][BMA2X2] " x)
#define E(x...) printk(KERN_ERR "[GSNR][BMA2X2] " x)

#define ACC_NAME  "bma2x2"
...
#define DEFAULT_TAP_JUDGE_PERIOD 1000    
#endif
...
static void bma2x2_work_func(struct work_struct *work)
{
	...
	acc.x += bma2x2->offset_buf[0];
	acc.y += bma2x2->offset_buf[1];
	acc.z += bma2x2->offset_buf[2];
		...
		int en)
{
	...
	if (en) {
		...
		err += bma2x2_set_Int_Enable(client, 5, 1);
		err += bma2x2_set_Int_Enable(client, 6, 1);
		err += bma2x2_set_Int_Enable(client, 7, 1);
	#ifdef BMA2X2_ENABLE_INT1
		...
		err = bma2x2_set_Int_Enable(client, 5, 0);
		err += bma2x2_set_Int_Enable(client, 6, 0);
		err += bma2x2_set_Int_Enable(client, 7, 0);
	}
	return err;
}
		...
		const char *buf, size_t count)
{
	...
	bma2x2->pdata->gs_kvalue = simple_strtoul(buf, NULL, 10);
	D("%s: bma2x2->pdata->gs_kvalue = 0x%x\n", __func__, bma2x2->pdata->gs_kvalue);

	if ((bma2x2->pdata->gs_kvalue & (0x67 << 24)) != (0x67 << 24)) {
		bma2x2->offset_buf[0] = 0;
		bma2x2->offset_buf[1] = 0;
		bma2x2->offset_buf[2] = 0;
	} else {
		bma2x2->offset_buf[0] = (bma2x2->pdata->gs_kvalue >> 16) & 0xFF;
		bma2x2->offset_buf[1] = (bma2x2->pdata->gs_kvalue >>  8) & 0xFF;
		bma2x2->offset_buf[2] =  bma2x2->pdata->gs_kvalue        & 0xFF;
...
#endif
static DEVICE_ATTR(set_k_value, S_IRUSR|S_IRGRP|S_IWUSR,
			...
			struct bma2x2_platform_data *pdata)
{
	...
	pdata->gs_kvalue = 0;
	if ((np = of_find_node_by_path(CALIBRATION_DATA_PATH))) {
		cali_data = (unsigned char*) of_get_property(np, G_SENSOR_FLASH_DATA, &cali_size);
		...
		const struct i2c_device_id *id)
{
	...
	if ((data->pdata->gs_kvalue & (0x67 << 24)) != (0x67 << 24)) {
		data->offset_buf[0] = 0;
		data->offset_buf[1] = 0;
		data->offset_buf[2] = 0;
	} else {
		data->offset_buf[0] = (data->pdata->gs_kvalue >> 16) & 0xFF;
		data->offset_buf[1] = (data->pdata->gs_kvalue >>  8) & 0xFF;
		data->offset_buf[2] =  data->pdata->gs_kvalue        & 0xFF;

		for (i = 0; i < 3; i++) {
			if (data->offset_buf[i] > 127) {
				data->offset_buf[i] =
					data->offset_buf[i] - 256;
			}
		}
	}
