...
#define CONFIG_USB_EP_LIMITATION

int rom_stockui = 0;

#ifdef CONFIG_MACH_A11_UL
#define USB_FUNC_ADB_RNDIS ((1 << USB_FUNCTION_ADB) | (1 << USB_FUNCTION_RNDIS))
#define USB_FUNC_ADB_IPT ((1 << USB_FUNCTION_ADB) | (1 << USB_FUNCTION_RNDIS_IPT))
#endif
...
struct android_usb_function *get_specific_usb_function(char *name)
{
	struct android_dev *dev = _android_dev;
...
int check_special_order_combination(unsigned func, struct list_head *enable_list,int *product_id,int *vendor_id)
{
	struct android_dev *dev = _android_dev;
	struct android_usb_product *p = speical_usb_products_list;
	struct android_usb_function *f;
	int count = ARRAY_SIZE(speical_usb_products_list);
	char ** functions_name;
	int i,j;

	for (i = 0; i < count; i++, p++) {
		if (func == get_string_array_value(p->functions,p->num_functions)) {
			functions_name = p->functions;
			*product_id = p->product_id;
			*vendor_id = p->vendor_id;
			for (j = 0;j < p->num_functions;j++) {
				f = get_specific_usb_function(*functions_name);
				functions_name++;
				if (f == NULL) {
					
					printk(KERN_WARNING "[USB] %s : fail to get usb function %s\n",__func__,*functions_name);
					
					return -EINVAL; 
				}
				if (android_usb_function_holder_list_add_tail(f, enable_list, dev)) {
					printk(KERN_WARNING "[USB] %s : Cannot add %s\n",__func__, f->name);
					return -EINVAL;
				}
			}
			return 0;
...
int android_switch_function(unsigned func)
{
	...
	if (dev->enabled != true) {
		printk(KERN_WARNING "[USB] %s: USB driver is not initialize\n", __func__);
		mutex_unlock(&function_bind_sem);
		return 0;
	}
	...
	printk(KERN_WARNING "[USB] %u, before %u\n", func, val);

	if (func == val) {
		printk(KERN_WARNING "[USB] %s: SKIP due the function is the same ,%u\n" , __func__, func);
		mutex_unlock(&function_bind_sem);
		return 0;
	}
#ifdef CONFIG_MACH_A11_UL
	if (func & (1 << USB_FUNCTION_AUTOBOT)) {
		func &= ~((1 << USB_FUNCTION_UMS) | (1 << USB_FUNCTION_ADB));
		printk(KERN_WARNING "[USB] Projector specific %u, before %u\n", func, val);
	} else if (((func & USB_FUNC_ADB_RNDIS) == USB_FUNC_ADB_RNDIS
			|| (func & USB_FUNC_ADB_IPT) == USB_FUNC_ADB_IPT)
			&& !(func & (1 << USB_FUNCTION_AUTOBOT))) {
		func &= ~(1 << USB_FUNCTION_UMS);
		printk(KERN_WARNING "[USB] IPT/rndis specific %u, before %u\n", func, val);
	}
#endif

	if ((!(get_radio_flag() & BIT(17))) && (mode == MFG_MODE_MFGKERNEL || mode == MFG_MODE_MFGKERNEL_DIAG58) && (func & (1 << USB_FUNCTION_MODEM))) {
		printk(KERN_WARNING "[USB] mfg mode & 8 20000 are enabled. don't enable modem interface\n");
		mutex_unlock(&function_bind_sem);
		return 0;
	}
	...
	if (product && dev->pdata) {
		...
		printk(KERN_WARNING "[USB] vendor/product id");
	...
	if (rom_stockui && (product_id == PID_MTPUMS)) {
		...
		printk(KERN_WARNING "[USB] %s: stockUI ROM\n", __func__);
	}

	printk(KERN_WARNING "[USB] %s: rom_stockui=%d\n", __func__, rom_stockui);

	printk(KERN_WARNING "[USB] %s: vendor_id=0x%x, product_id=0x%x\n",
...
static ssize_t show_usb_car_kit_enable(struct device *dev, struct device_attribute *attr,
		char *buf)
{
	unsigned length;
	int value = 0;
...
static DEVICE_ATTR(host_mode, 0220,NULL, store_usb_host_mode);
#if 0
static DEVICE_ATTR(usb_phy_setting, 0664, show_usb_phy_setting, store_usb_phy_setting);
#endif
static DEVICE_ATTR(ats, 0664, show_ats, NULL);
...
static void setup_vendor_info(struct android_dev *dev) {
	...
	if (get_radio_flag() & BIT(17)) {
		...
		product_id = 0x0ff3;
		goto bind;
	} else if (dev->pdata->specific_rom_cb()) {
		ANDROID_USB_ENABLE_FUNC(dev, conf, "diag");
		ANDROID_USB_ENABLE_FUNC(dev, conf, "modem");
		ANDROID_USB_ENABLE_FUNC(dev, conf, "rmnet");
		vendor_id = 0x0bb4;
