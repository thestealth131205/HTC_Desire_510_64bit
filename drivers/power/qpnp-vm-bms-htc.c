...
struct pm8916_battery_data_store {
	int           store_soc;
	unsigned long store_currtime_ms;
};
static struct pm8916_battery_data_store store_emmc;
...
static bool is_battery_charging(struct qpnp_bms_chip *chip)
{
	...
	charge_type = pm8916_get_charge_type();
	if (charge_type < 0) {
		pr_info("pm8916_get_charge_type not ready.\n");
		return false;
	} else {
		return (pm8916_get_charge_type() != POWER_SUPPLY_CHARGE_TYPE_NONE);
	}
}
...
static bool is_battery_present(struct qpnp_bms_chip *chip)
{
	...
	return pm8916_get_batt_present();
}
...
{
	int batt_status = pm8916_get_batt_status();

	if (batt_status == POWER_SUPPLY_STATUS_UNKNOWN) {
		pr_err("Unable to get battery 'STATUS'\n");
		return -EINVAL;
...
static int dump_all(void)
{
	...
	len += scnprintf(batt_log_buf + len, BATT_LOG_BUF_LEN - len,
			"OCV=%d,", the_chip->last_ocv_uv);

	len += scnprintf(batt_log_buf + len, BATT_LOG_BUF_LEN - len,
			"IBAT=%d,", the_chip->current_now);

	len += scnprintf(batt_log_buf + len, BATT_LOG_BUF_LEN - len,
			"BMS FSM=%d,", the_chip->current_fsm_state);

	for (i = 0; i < the_chip->bms_data.num_fifo; i++) {
		len += scnprintf(batt_log_buf + len, BATT_LOG_BUF_LEN - len,
			"FIFO_uv[%d]=%d,", i, the_chip->bms_data.fifo_uv[i]);
	}
...
inline int pm8916_bms_dump_all(void)
{
	if (!the_chip) {
...
int pm8916_gauge_get_attr_text(char *buf, int size)
{
	...
	len += scnprintf(buf + len, size - len,
			"IBAT: %d;\n", the_chip->current_now);

	len += scnprintf(buf + len, size - len,
			"BMS_FSM: %d;\n", the_chip->current_fsm_state);

	for (i = 0; i < the_chip->bms_data.num_fifo; i++) {
		len += scnprintf(buf + len, size - len,
			"FIFO_uv[%d]: %d;\n", i, the_chip->bms_data.fifo_uv[i]);
	}

	len += pm8916_batterydata_ioctl_attr_text(buf + len, size - len);
...
static void pm8916_btm_voltage_alarm_notify(enum qpnp_tm_state state, void *ctx)
{
	...
	if (state == ADC_TM_LOW_STATE) {
		...
		if (vbat_uv - VBATT_ERROR_MARGIN
			...
			pm8916_batt_lower_alarm_threshold_set(0);
...
int pm8916_batt_lower_alarm_threshold_set(int threshold_mV)
{
	...
	the_chip->vbat_monitor_params.threshold_notification = &pm8916_btm_voltage_alarm_notify;
...
int pm8916_bms_consistence_check(void)
{
	...
	if (the_chip->warm_reset && the_chip->dt.batt_stored_magic_num == BMS_STORE_MAGIC_NUM
			&& the_chip->dt.store_batt_data_soc_thre > 0 && batt_temp >= 0
			&& (the_chip->calculated_soc > the_chip->dt.batt_stored_soc
			|| the_chip->dt.batt_stored_soc - the_chip->calculated_soc > 1)
			&& (currtime_ms - the_chip->dt.batt_stored_update_time) < 3600000) {
			consistent_flag = true;
	}

	pr_info("warm_reset:%d,batt_stored_magic_num:0x%X,store_batt_data_soc_thre:%d,batt_temp:%d,"
		"calculated_soc:%d,batt_stored_soc:%d,currtime_ms:%lu,store_time:%u,consistent_flag:%d\n",
		the_chip->warm_reset, the_chip->dt.batt_stored_magic_num, the_chip->dt.store_batt_data_soc_thre,
		batt_temp, the_chip->calculated_soc, the_chip->dt.batt_stored_soc, currtime_ms,
		the_chip->dt.batt_stored_update_time,consistent_flag);
...
int pm8916_bms_vddmax_by_batterydata(void) {
	...
	if (the_chip->batt_data->vddmax_mv != -EINVAL)
		return the_chip->batt_data->vddmax_mv;
	else
		return 0;
}
