...
static int param_set_input_boost_freq(const char *val, struct kernel_param *kp);
static int param_get_input_boost_freq(char *buffer, struct kernel_param *kp);
...
static unsigned int input_boost_freq[CONFIG_NR_CPUS] = { 0 };
module_param_call(input_boost_freq, param_set_input_boost_freq, param_get_input_boost_freq, &input_boost_freq, 0644);
...
static int arch_msm_cortexmp = 0;
...
static int param_set_input_boost_freq(const char *val, struct kernel_param *kp)
{
	...
	for (i=0; i<CONFIG_NR_CPUS; i++) {
		...
		value = simple_strtoul(token, NULL, 10);
		input_boost_freq[i] = value;
	}

	for(j=i; j<CONFIG_NR_CPUS; j++)
	{
		input_boost_freq[j] = value;
	}
...
static int param_get_input_boost_freq(char *buffer, struct kernel_param *kp)
{
	...
	for (i=0; i<CONFIG_NR_CPUS -1; i++) {
		len = strlen(buffer);
		sprintf(buffer+len, "%d,", input_boost_freq[i]);
	}

	len = strlen(buffer);
	sprintf(buffer+len, "%d", input_boost_freq[CONFIG_NR_CPUS -1]);
...
static int do_input_boost(void *data)
{
	...
	while (1) {
		...
		ret = cpufreq_get_policy(&policy, smp_processor_id());
		if (ret)
			goto bail_incorrect_governor;

		i_sync_info = &per_cpu(sync_info, smp_processor_id());

		cancel_delayed_work_sync(&i_sync_info->input_boost_rem);

		if (policy.cur < i_sync_info->input_boost_min)
		...
		unsigned int type, unsigned int code, int value)
{
	...
	boost_freq = input_boost_freq[num_online_cpus() - 1];
...
static int cpu_boost_init(void)
{
	...
	for_each_possible_cpu(cpu) {
		...
		pthread = kthread_create_on_node(do_input_boost,
		...
		if (!ret && cpumask_weight(policy.related_cpus) > 1) {
			arch_msm_cortexmp = 1;
			break;
		}
	}
