...
#if defined(CONFIG_HTC_POWER_DEBUG) && defined(CONFIG_PINCTRL_MSM_TLMM_V4)
...
void __msm_gpio_get_dump_info(struct gpio_chip *gc, unsigned gpio, struct msm_gpio_dump_info *data)
{
	...
	cfg_dump = readl_relaxed(TLMMV4_GP_CFG(pinfo->reg_base, gpio));
        ...
        if (data->dir)
                data->value = (readl_relaxed(TLMMV4_GP_INOUT(pinfo->reg_base, gpio)) >> 1) & 0x1;
        else {
                data->value = readl_relaxed(TLMMV4_GP_INOUT(pinfo->reg_base, gpio)) & 0x1;
                data->int_en = readl_relaxed(TLMMV4_GP_INTR_CFG(pinfo->reg_base, gpio)) & 0x1;
                if (data->int_en)
                        data->int_owner = (readl_relaxed(TLMMV4_GP_INTR_CFG(pinfo->reg_base, gpio)) >> 5) & 0x7;
        }
